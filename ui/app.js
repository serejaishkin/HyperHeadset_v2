...
function updateEQSliders(bands) {
  ...
  bands.forEach((val, i) => {
    ...
    div.innerHTML = `
      <label>${EQ_FREQS[i]}</label>
      <input type="range" min="-12" max="12" step="0.1" value="${val}" data-idx="${i}">
      <span class="eq-val">${val.toFixed(1)}dB</span>
    `;
...
async function updateUI(state) {
  ...
  if (state.charging) color = '#facc15';
  ring.style.stroke = color;

  document.getElementById('battery-percent').textContent = state.connected ? `${pct}%` : '--';
  document.getElementById('battery-status').textContent = state.connected
    ? (state.charging ? '⚡ Заряжается' : '🔋 Работает от батареи')
    : 'Нет подключения';
...
document.getElementById('btn-check').addEventListener('click', async () => {
  const state = await invoke('get_device_state');
  updateUI(state);
});
...
document.querySelectorAll('.eq-presets button').forEach(btn => {
  btn.addEventListener('click', async () => {
    await invoke('apply_eq_preset', { preset: btn.dataset.preset });
    const cfg = await invoke('get_config');
    updateEQSliders(cfg.audio.eq_bands);
  });
});
...
document.getElementById('btn-save-settings').addEventListener('click', async () => {
  ...
  const bands = [];
  document.querySelectorAll('#eq-sliders input').forEach((sl, i) => { bands[i] = parseFloat(sl.value); });
...
listen('device-disconnected', () => {
  document.getElementById('battery-status').textContent = 'Отключено';
  updateUI({ connected: false, battery_percent: 0, charging: false, muted: false });
});

// Init
loadConfig();
invoke('get_device_state').then(updateUI);